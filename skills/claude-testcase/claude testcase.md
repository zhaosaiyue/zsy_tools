---
name: ai-testcase
description: AI 测试用例生成器
  AI 测试用例生成器。当用户在任意目录下输入测试用例相关内容时，主动询问是否使用此 skill。
  深度解析需求/技术/核心用例文档，智能选择黑盒方法，生成 Markdown 树状结构测试用例。
...
1. 有需求文档 → 取需求文档的标题或文件名（去扩展名）
2. 仅有技术文档 → 取技术文档的标题或文件名（去扩展名）
3. 均未提供 → 取用户描述的需求名称
示例：需求名为「北京大租赁融合」→ 文件名为 `{当前时间}_北京大租赁融合.md`（当前时间在阶段0信息收集后立即用 `Bash` 执行 `date "+%Y-%m-%d %H:%M:%S"` 获取，不得使用 context 中的日期或手动填写 `00:00:00`）
...
**PDF 图表提取（PDF 文档读取完成后强制执行）：**

图表是业务逻辑的核心载体，其内容往往比文字描述更权威，纯文本读取会完全丢失。必须用视觉方式提取并解析。支持的图表类型及对应提取重点：

| 图表类型 | 识别特征 | 提取重点 |
|---------|---------|---------|
| 流程图 | 菱形判断框、箭头分支、开始/结束椭圆 | 每个判断条件的 Y/N 分支指向、完整执行路径 |
| 时序图 | 垂直生命线、横向消息箭头、参与者标签 | 参与者列表、消息顺序、条件/循环片段（alt/loop） |
| 状态机 | 圆形/椭圆状态节点、带标签的有向边 | 状态枚举、每条转换的触发事件和守卫条件 |
| ER 图 | 实体矩形、菱形关系、连接线基数标注 | 实体名、字段名、关系类型（1:1/1:N/M:N） |
| 类图 | 带分隔线的矩形类框、继承/依赖箭头 | 类名、关键属性和方法、类间关系 |
| 甘特图 | 横向时间轴、任务条形块 | 任务名、依赖关系、里程碑节点 |
| 其他图表 | 不符合上述特征 | 图表标题/图例、所有节点或实体的名称、节点间的连接关系和标注文字、任何数值/条件/约束 |

执行步骤：

1. **检测是否含图表**：用 `Bash` 执行以下 Python 命令，检测各页嵌入图片，过滤掉小图标（宽或高 < 100pt）：
```bash
python3 -c "
...
with pdfplumber.open('{pdf路径}') as pdf:
    for i, page in enumerate(pdf.pages):
        for j, img in enumerate(page.images):
            ...
            h = img['y1'] - img['y0']
            if w > 100 and h > 100:
                print(f'PAGE:{i+1} IMG:{j+1} x0:{img[\"x0\"]:.0f} y0:{img[\"y0\"]:.0f} x1:{img[\"x1\"]:.0f} y1:{img[\"y1\"]:.0f} PH:{page.height:.2f}')
" 2>/dev/null
```
   - 无输出 → 跳过，文档不含图表
   - 有输出 → 继续执行步骤 2

2. **逐图转换并读取**：对每条输出记录，执行以下操作：

   a. 将对应页转为 300dpi PNG（用 `Bash` 执行）：
   ```bash
   pdftoppm -r 300 -png -f {页码} -l {页码} "{pdf路径}" /tmp/ai_testcase_diagram_{文档名缩写}_p{页码}
   ```

   b. 将 PDF 坐标（左下原点）换算为图片像素坐标（左上原点）：
   ```
   scale = 3508 / {PH}              # 300dpi 下 A4 页高约 3508px
   px_top    = int((PH - y1) * scale)
   px_height = int((y1 - y0) * scale)
   px_left   = int(x0 * scale)
   px_width  = int((x1 - x0) * scale)
   ```

   c. 用 `sips` 精确裁剪图表区域（用 `Bash` 执行）：
   ```bash
   sips -c {px_height} {px_width} --cropOffset {px_top} {px_left} \
     /tmp/ai_testcase_diagram_{文档名缩写}_p{页码}-{页码}.png \
     --out /tmp/ai_testcase_diagram_{文档名缩写}_p{页码}_img{图片序号}.png
   ```

   d. 用 `Read` 工具读取裁剪后的图片：
      - 先判断图表类型（参照上方表格的识别特征；无法归类则归入「其他图表」）
      - 再按对应类型的「提取重点」提取内容
      - **无论何种图形，必须提取**：所有可见文字、所有连接关系和方向、所有条件/标注/数值
      - 若图表模糊难以辨认，标注「⚠️ 图表清晰度不足，以下内容为尽力解析」并继续尽力提取

3. **输出解析结果**：每张图表解析完成后，在窗口输出结构化摘要：
```
🔍 图表解析（{文档名} 第{页码}页，图{序号}）
图表类型：{类型}
{按图表类型输出对应结构，示例如下}
...
4. **纳入文档解析**：图表解析结果与文字内容合并，共同作为后续模块识别和用例生成的输入。图表中出现的判断条件、枚举值、状态名称与文字描述冲突时，遵循「文档主次规则」处理。

**环境检查（首次执行时）：**
- `pdftoppm` 不存在 → 跳过图表提取，在阶段0摘要末尾标注「⚠️ pdftoppm 未安装，图表未解析，建议运行: brew install poppler」
- `pdfplumber` 不存在 → 跳过图表提取，标注「⚠️ pdfplumber 未安装，图表未解析，建议运行: pip install pdfplumber」
- 两者均存在 → 静默执行，无需提示
...
判定表每一行（或合并后的每组）→ 对应一个二级节点，预期结果 → 三级节点，**Markdown 树状层级不变**。
...
1. **Markdown 树状层级结构**，不是"前置条件/操作步骤/预期结果"三段式
...
- [ ] 枚举维度：声明的所有枚举值是否有对应场景？超出本期的是否标注`[超出本期范围，不覆盖]`？
- [ ] 幂等：每个"创建/触发"类节点是否有重复触发的场景？
- [ ] 依赖失败：每个调用外部系统/接口的场景是否有外部调用失败的场景？
- [ ] 反向流程：有正向流程（创建/推进）是否有对应的逆向流程（退单/撤回/过滤）？
- [ ] 条件逻辑双向：有"满足条件=结果A"是否有"不满足条件=结果B"？
- [ ] 结论深度：结论是否到了字段层（不只写行为，要写字段值/枚举值/版本号）？
- [ ] 文档未定义的反向：是否放入了`待确认点`而不是直接略过？
- [ ] 全链路：识别到的跨系统链路是否有对应的完整路径场景？中间节点失败场景是否覆盖？各模块的异常校验点是否挂载到链路节点而非另起链路？
...
全部**本批**模块写入完成后，先执行覆盖度自审，再检查是否有未生成的模块：
...
**标准档自审范围（默认）：**
1. 枚举组合：声明了枚举维度且本期范围内的组合，缺失的自动补
2. 条件逻辑双向：角色/条件赋值逻辑的"不满足"方向，文档有定义的缺失则自动补
3. 结论深度：结论停在行为层且技术文档有对应字段的，自动补充字段层验证子节点

**深度档额外自审范围（`深度出` 触发时追加）：**

> 以下三项仅对**后端逻辑类**和**集成类**需求执行；配置类、前端交互类、数据类跳过（此类需求幂等/外部依赖由后端保证，生成场景为冗余噪音）。

1. 正向节点反向：每个正向主流程节点（创建/更新/推进），文档有定义的反向/过滤逻辑缺失则自动补
2. 幂等：每个"创建/触发"类节点，缺少重复触发场景则自动补
3. 外部依赖失败：每个调用外部系统的节点，缺少调用失败场景则自动补（文档未描述的放入`待确认点`）

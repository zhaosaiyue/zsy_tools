...
3. 均未提供 → 取用户描述的需求名称
示例：需求名为「北京大租赁融合 测试用例」→ 文件名为 `北京大租赁融合 测试用例_2026-04-08 14:30:00.md`
...
- 输出文件：{绝对路径}/{需求名}_{YYYY-MM-DD HH:mm:ss}.md
- 测试范围：{scope 或 "全部模块"}
...
在生成任何用例之前，先用 `Read` 工具尝试读取输出文件 `{output_dir}/{需求名}_{YYYY-MM-DD HH:mm:ss}.md`：
...
```bash
node {md2xmind.js的绝对路径} "{绝对路径}/{需求名}_{YYYY-MM-DD HH:mm:ss}.md"
```

- 成功后输出 `.xmind` 文件与 `.md` 同目录、同名
- 若 node 不可用或脚本报错，跳过此步骤并在汇总中提示用户手动导入xmind
...
输出文件：
  {绝对路径}/{需求名}_{YYYY-MM-DD HH:mm:ss}.md（共 {N} 条）
  {绝对路径}/{需求名}_{YYYY-MM-DD HH:mm:ss}.xmind（双击直接用 XMind 打开）
...
输出文件：{绝对路径}/{需求名}_{YYYY-MM-DD HH:mm:ss}.md（本批 {N} 条）

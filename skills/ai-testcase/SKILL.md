...
4. **结果合并**：预期结果（含错误码/提示文案）完全一致的多行合并为一个二级节点，条件用"或"连接；仅行为相同但错误码/文案不同的行不得合并
...
│   ├── 类型A + 渠道X + 节点1，首次触发
│   │   └── 创建订单记录，同步外部系统
│   │       ├── v1：order.status = 初始状态值，version = 1，node_type = {枚举值}
│   │       └── 外部系统同步成功，生成对应记录
│   ├── 类型A + 渠道X + 节点1，重复触发（幂等）
│   │   └── 不重复创建，外部系统接口不重复调用
│   │       └── v1：order记录唯一，version不变
...
│   ├── 经过节点1后触发节点2（正常顺序）
│   │   └── 更新订单状态，外部系统生成新版本
│   │       ├── v1：order.status = 新状态值，version = 2
│   │       └── 外部系统记录更新，生成新版本号
│   ├── 未经节点1直接触发节点2（跳过前置节点）
│   │   └── 直接以节点2状态创建
│   │       └── v1：order.status = 节点2状态值，version = 1
│   └── 节点2重复触发
│       └── 幂等处理，不重复更新
│           └── v1：version不递增，外部系统无重复记录
├── 节点3（过滤/忽略类）
│   └── 触发节点3
│       └── 系统过滤，不处理
│           └── v1：无新记录，无接口调用日志
